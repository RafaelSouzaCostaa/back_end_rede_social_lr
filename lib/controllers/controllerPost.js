...
module.exports = {
    ...
    getAll: async function (req, res, next) {
        ...
        */
        try {
            let profile = await Profile.findById(req._idToken);
    ...
    getAllByFollow: async function (req, res, next) {
        ...
        */
        try {
            ...
            let postsFollow = [];
            let profile = await Profile.findById(id);
            let idsFollow = profile.followingObjectId;

            if (profile != null) {
                for (let idFollow of idsFollow) {
                    profile = await Profile.findById(idFollow._id);
                    if (!profile) {
                        continue;
                    }
    ...
    like: async function (req, res, next) {
        /**
         * #swagger.tags = ["Post"]
         * #swagger.summary = 'Adiciona ou Remove uma Curtida da Postagem indicada'
             #swagger.parameters['Post'] = { 
                in: 'body', 
                required: true, 
                schema: { 
