...
module.exports = {
    ...
    like: async function (req, res, next) {
        ...
        try {
            ...
            let isEnjoying = true;
            profileLiked.likedPosts.forEach((postLike) => {
            ...
            let profilePostOwner = await Profile.findById(
                req.body.profileObjectId
            );
            if (!profilePostOwner) {
                res.status(404).json({
                    msg: "profile não encontrado",
            ...
            if (isEnjoying) {
                ...
                profilePostOwner.posts.id(
                    req.body.postObjectId
                ).numberOfLikes += 1;
