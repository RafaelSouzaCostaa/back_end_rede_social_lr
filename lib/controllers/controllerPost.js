...
module.exports = {
    create: async function (req, res, next) {
        /**
         * #swagger.tags = ["Post"]
         * #swagger.summary = 'Adiciona uma nova Postagem ao Banco de Dados'
    ...
    like: async function (req, res, next) {
        /**
         * #swagger.tags = ["Post"]
         * #swagger.summary = 'Salva/Remove uma Curtida'
             #swagger.parameters['Post'] = { 
                in: 'body', 
                required: true, 
                schema: { 
         ...
         */
        //TESTADO esta tudo funcionando
        //LUIGGI criado agora - atualziar documentation - e outros detalhes que precisar
        ...
        try {
            ...
            if (!profileLiked) {
                res.status(404).json({
                    msg: "profile não encontrado",
                });
            }

            let isEnjoying = true;
            profileLiked.postsLike.forEach((postLike) => {
                if (postLike.postObjectId == post.postObjectId) {
                    isEnjoying = false;
                    msg = "Descurtiu";
                    return;
                }
            ...
            let profilePost = await Profile.findById(req.body.profileObjectId);
            if (!profilePost) {
                res.status(404).json({
                    msg: "profile não encontrado",
                });
            ...
            if (isEnjoying) {
                profileLiked.postsLike.push(post);

                profilePost.posts
                    .id(req.body.postObjectId)
                    .postLikedBy.push(_idToken);

                profilePostOwner.posts.id(postObjectId).numberOfLikes += 1;
                profilePostOwner.save();
                profileLiked.save();
                res.status(200).json({ msg: msg });
            } else {
                profileLiked.postsLike = profileLiked.postsLike.filter(
                    (post) => {
                ...
                profilePost.posts.id(req.body.postObjectId).numberOfLikes -= 1;
                profilePost.save();
                profileLiked.save();
                res.status(200).json({ msg: msg });
            }
