...
describe("Testes de Perfil", () => {
    ...
    it("Inserir Comentario em Postagem", (done) => {
        let newComment = {
            ...
            creationDate: Date.now(),
            postObjectId: test_post._id
        };
        chai.request(host)
            .post(`/comment/create/${test_post.posts[0]._id}`)
            .send(newComment)
            .set("x-access-token", token)
            .end((err, res) => {
                ...
                expect(res.body[0]).to.have.property("_id");
                // expect(res.body[0]).to.have.property("posts");
    ...
    //     chai.request(host)
    //         .delete(`/post/deleteById/${test_profile._id}/${test_post.posts[0]._id}`)
    //         .set("x-access-token", token)
    //         .end((err, res) => {
    //             expect(res).to.have.status(200);
