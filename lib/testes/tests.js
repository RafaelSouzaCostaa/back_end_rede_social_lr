...
describe("Testes de Perfil", () => {
    ...
    it("Primeiro perfil deve seguir segundo Perfil", (done) => {
        chai.request(host)
            ...
            .end((err, res) => {
                expect(res).to.have.status(200);
                expect(res.body).to.have.property("msg");
                expect(res.body).to.have.property("profileToken");
                expect(res.body).to.have.property("followProfile");
                expect(res.body.msg).to.be.equal("seguindo");
                expect(res.body.profileToken.followingObjectId).to.not.be.empty;
                expect(res.body.profileToken.followersObjectId).to.be.empty;
                expect(res.body.followProfile.followingObjectId).to.be.empty;
                expect(res.body.followProfile.followersObjectId).to.not.be
                    .empty;
                done();
            });
    });
    ...
    it("Primeiro perfil deve parar seguir segundo Perfil", (done) => {
        chai.request(host)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.have.property("profileToken");
                expect(res.body).to.have.property("followProfile");
                expect(res.body.msg).to.be.equal("removido");
                expect(res.body.profileToken.followingObjectId).to.be.empty;
                expect(res.body.profileToken.followersObjectId).to.be.empty;
                expect(res.body.followProfile.followingObjectId).to.be.empty;
                expect(res.body.followProfile.followersObjectId).to.be.empty;
                done();
            });
    });
    ...
    it("Perfil deve curtir a Publicação", (done) => {
        chai.request(host)
            .post("/post/like")
            .send({
                profileObjectId: test_profile._id,
            ...
            .end((err, res) => {
                expect(res).to.have.status(200);
                expect(res.body).to.have.property("msg");
                expect(res.body.msg).to.be.equal("Curtiu");
                expect(res.body.likedPost.numberOfLikes).to.be.equal(1);
                expect(res.body.likedPost.likes[0]).to.be.equals(test_profile._id);
                done();
            });
    });

    // it("Perfil deve descurtir a Publicação", (done) => {
    //     chai.request(host)
    //         .post("/post/like")
    //         .send({
    //             profileObjectId: test_profile._id,
    ...
    //             expect(res).to.have.status(200);
    //             expect(res.body).to.have.property("msg");
    //             expect(res.body.msg).to.be.equal("Descurtiu");
    //             expect(res.body.likedPost.numberOfLikes).to.be.equal(0);
    //             expect(res.body.likedPost.likes[0]).to.be.empty;
    //             done();
    //         });
    // });
    ...
    it("Inserir Comentario em Postagem", (done) => {
        ...
        chai.request(host)
            ...
            .end((err, res) => {
                ...
                expect(res.body).to.be.an("object");
                expect(res.body).to.have.property("_id");
                expect(res.body).to.have.property("postObjectId");
                expect(res.body).to.have.property("profileObjectId");
                expect(res.body).to.have.property("comment");
                expect(res.body).to.have.property("creationDate");
                test_comment = res.body;
                done();
            });
    });
    ...
    it("Comentario deletado", (done) => {
        chai.request(host) // "/deleteById/:profileObjectId/:commentObjectID"
            .delete(
                `/comment/deleteById/${test_profile._id}/${test_comment._id}`
            )
            .set("x-access-token", token)
            .end((err, res) => {
                expect(res).to.have.status(200);
    ...
    it("Postagem deve ser deletada", (done) => {
        chai.request(host)
            .delete(
                `/post/deleteById/${test_profile._id}/${test_profile.posts[0]._id}`
            )
            .set("x-access-token", token)
            .end((err, res) => {
                expect(res).to.have.status(200);
